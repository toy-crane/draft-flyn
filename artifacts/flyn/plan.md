...
- **수용 기준**:
  - [ ] 시나리오 0개 상태에서 사이드바의 "내 시나리오" / "히스토리" 섹션 헤더가 보이지 않는다
  - [ ] 메인 영역에 "첫 시나리오를 만들어보세요" CTA가 보인다
  - [ ] CTA 아래에 예시 칩 3-5개가 표시된다 ("카페 주문", "면접 연습", "공항 환승", "병원 예약", "마트 장보기")
  - [ ] "첫 시나리오 만들기" CTA 또는 "+ 새 시나리오" 클릭 → `/scenarios/new`로 이동 (router push)
  - [ ] 시나리오에 진행 중 인스턴스가 있으면 사이드바 항목 우측에 ▸ 인디케이터가 표시된다 (없으면 표시 안 됨)
  - [ ] 데스크탑 폭(≥768px)에서 사이드바 접기 버튼 클릭 → 사이드바 폭이 줄거나 사라진다
  - [ ] 사이드바 접힘/펼침 상태가 페이지 이동 후에도 세션 내 유지된다 (`localStorage` 또는 cookie)
  - [ ] 모바일 폭(<768px)에서는 햄버거 아이콘이 보이고, 클릭 시 사이드바가 오버레이로 나타난다
  - [ ] 모바일 오버레이 바깥 탭 → 사이드바가 닫힌다
  - [ ] 모바일 폭에서 사이드바 안의 시나리오/히스토리 항목 클릭 → 사이드바가 자동으로 닫히고 메인 영역이 해당 화면으로 전환된다
- **검증**:
